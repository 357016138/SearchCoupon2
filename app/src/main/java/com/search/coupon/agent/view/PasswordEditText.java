...
/**
 * Created by RickBerg on 2018/2/12 0012.
 *
 */

public class PasswordEditText extends android.support.v7.widget.AppCompatTextView {
    // 画笔
    ...
    public void addPassword(int number) {
        String strPwd = getText().toString().trim() + number;
        if (strPwd.length() > mPasswordNumber) {
            return;
        }
        setText(strPwd);
    }
