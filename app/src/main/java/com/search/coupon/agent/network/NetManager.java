...
import com.search.coupon.agent.utils.AESUtils;
...
public class NetManager {
    ...
    public Call postJson(Context context, RequestParams params, Callback callback) {
        ...
        String requestJson = handleParam(context, params);
        Request.Builder requestBuilder = new Request.Builder().url(params.getUrl());
    ...
    private String handleParam(Context context, RequestParams params) {
        ...
        String requestJson = GsonUtil.toJson(bodyMap);
        LogUtils.e("请求参数加密前--" + params.getUrl() + requestJson);
        if (params.isEncrypt()) {
            TreeMap encryptMap = new TreeMap();
            encryptMap.put("aesRequest", AESUtils.aesEncryptStr(requestJson, AESUtils.AES_SECRET));
