...
public class ResetPasswordActivity extends BaseActivity implements View.OnFocusChangeListener {
    ...
    public void onClickEvent(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_del_oldpassword:
                resetPassWord_oldPassword_input.setText("");
                break;
            case R.id.iv_del_password:
                resetPassWord_password_input.setText("");
                break;
            case R.id.iv_del_confirmpassword:
                resetPassWord_ConfirmPassWord.setText("");
                break;
            default:
                break;
    ...
    private void getDataFromView() {
        ...
        if (passWordOldStr == null || passWordOldStr.trim().length() <= 0) {
            ...
            boolean isTrue = UtilTools.checkPassWordInput(passWordOldStr);
            if (!isTrue) {
                toast("原密码必须为6-12位数字+字母组成");
                return;
            }
        }
